...
\end{cases}
$$
其中，$Y$ 也被称之为**有限损失** (limited loss variable)。右删失表示随机变量 $X>u$ 的所有取值都用 $u$ 代替

$Y$ 的数学期望 $E(X\wedge u)$ 称之为**有限期望** (limited expected value)，具体表示为：
$$
\begin{align*}
...
$$
其中，$\mu=\alpha/\beta$ 是均值参数，$\phi=1/\alpha$ 是离散参数。均值和方差分别为 $\mu$ 和 $\phi\mu^2$。
...
在非寿险精算中，通常用用计数随机变量来描述一段时间内发生的损失次数或索赔次数。计数随机变量是离散型随机变量的一种，随机变量的取值是在 $0,1,2, 3,4...$ 正整数值域。
...
【**例**】：假设随机变量 $X\sim \text{Gamma}(\alpha = 2, \theta = 100)$，其中期望为 200。
