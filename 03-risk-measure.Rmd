...
随机变量的生存函数(Survival Function)定义为： $$S_{X}(x)=\Pr(X>x)=1-F_{X}(x)$$其中，生存函数表示随机变量$X$大于$x$的概率。若随机变量 是连续的，其密度函数和生存函数存在下述关系： $$S_{X}(x)=\int_{0}^\infty f_{X}(x)dx$$
...
$$ 母函数可以用来随机变量$N=k$的概率，可以表示为母函数的k 阶偏导在0点处的取值，例如: $$P(N=1)=\frac{P_N^{(1)}(0)}{1!}$$ $$P(N=k)=\frac{P_N^{(k)}(0)}{k!}$$ 其中， $P_N^{(k)}(\cdot)$表示母函数的k阶偏导，$k!$表示 的阶乘。

定义：对于连续型随机变量 ，矩母函数 (Moment Generating Function, mgf) $$M_X(t)=E(e^{tX})=\int_{-\infty}^{\infty}e^{tX}f(x)dx$$

矩母函数可以得到多个独立随机变量之和的分布情况。假设$S=X_1+X_2+...+X_n=\sum_{i=1}^{n}X_i$ ，那么随机变量S的矩母函数表示为： $$M_s(t)=M_{X_1}(t)M_{X_2}(t)...M_{X_n}(t)$$
...
矩母函数可以用来计算原点矩，例如随机变量k阶原点矩等于矩母函数的k阶偏导在0点处的取值： $$E(X^k)=\frac{dM_X^{(k-1)}(t)}{dt}|_{t=0}=M_X^{(k)}(0)$$ 其中，$M_X^{(k)}(\cdot)$表示矩母函数的k阶偏导。
...
偏度 (Skewness)是对分布偏斜方向和程度的测度，若偏度系数大于0，则表示分布正偏/右偏，若偏度系数小于0，则表示分布负偏/左偏。具体定义w为三阶中心矩与标准差的三次方的比例： $$r_3=\frac{\mu_3}{\sigma^3}$$

(4)峰度 (Kurtosis)

峰度 (Kurtosis)是分布尖峰程度的测度。峰度系数越大，就意味着方差增大是由低频度的大于或小于平均值的极端差值引起的。 $$r_4=\frac{\mu_4}{\sigma^4}$$ 在更通常的情况下，峰度 (Kurtosis)定义为$r_4-3$，这也被称为超值峰度（Excess Kurtosis）。需要注意的是，正态分布的峰度系数为3， $r_4-3$是为了让正态分布的峰度为0。

(5)变异系数 (Coefficient of Variation)

变异系数 (Coefficient of Variation)是标准差和均值的比率，衡量分布离散程度的大小，可以用于消除量纲对结果的影响： $$r=\frac{(\mu_2)^\frac{1}{2}}{\mu'_1}=\frac{\sqrt{Var(X)}}{E(X)}$$
...
$$ 随机变量的方差表示为： $$Var(X)=E(X^2)-[E(X)]^2=360-18^2=36$$
