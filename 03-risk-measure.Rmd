...
在一段时间期间内，对损失发生的次数进行分析和预测是是精算和风险管理的第一步。在非寿险精算中，通常用用计数随机变量来描述一段时间内发生的损失次数。计数随机变量是离散型随机变量的一种，只在 0,1, 2, 3,4...正整数值域上取值。这一节主要介绍常用的三种损失次数分布：泊松分布（Poisson） 、二项分布（Binomial Distribution）和负二项分布（Negative Binomial distribution）。
...
### 5.3.3 TVaR （Tail-Value-at-Risk，尾部在险价值）
