...
# ======================================
  # 转移概率矩阵
  M.row1 <- c(0.1, 0.9, 0, 0) # 第一行的元素
  M.row2 <- c(0.1, 0, 0.9, 0)
  M.row3 <- c(0.1, 0, 0, 0.9)
  M.row4 <- c(0.1, 0, 0, 0.9)
  M <- (rbind(M.row1, M.row2, M.row3, M.row4))
  rownames(M) <- c('0%','20%', '40%', '50%')
  colnames(M) <- c('0%','20%', '40%', '50%')
  M
  ...
  a%*%M%*%M%*%M # 第三年后（稳态概率）
  a%*%M%*%M%*%M%*%M # 第四年后
  a%*%M%*%M%*%M%*%M%*%M # 三年以后加入稳态
  
# =====================================
# 课堂习题
# ======================================
  p0 <- dpois(0, lambda = 0.2)
  p1 <- dpois(1, lambda = 0.2)

  
  
  # 转移概率矩阵
  M.row1 <- c(1-p0, p0, 0) # 第一行的元素
  M.row2 <- c(1-p0, 0,  p0)
  M.row3 <- c(1-p0-p1, p1, p0)
  M <- (rbind(M.row1, M.row2, M.row3))
  rownames(M) <- c('0%','15%', '30%')
  colnames(M) <- c('0%','15%', '30%')
  round(M, 3)
  
  M.stability <- M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M%*%M
  M.stability <- M.stability[1,] #稳态概率
  M.stability*1000 # 每个保费等级的保单数
  M.stability*1000*c(1, 0.85, 0.7)*1000
  sum(M.stability*1000*c(1, 0.85, 0.7)*1000)
