...
$$
随机变量 $X_j$ 中未知参数 $\theta$ 的估计值求解步骤可分为以下几步：
...
例题：
已知随机变量服从指数分布，从中得到一个样本为：27，82，115，126，155，161，243，294，340，384，457，680，855，877，974，1193，1340，1884，2558，15743，求该指数分布的参数 $\theta$ 最大似然估计值。
...
\ell(\theta)&=\sum_{j=1}^n \log f_{X_j}(x_j|\theta).
\end{align*}

代入指数分布的概率密度函数，可得:
$$\hat{\theta}=\frac {\sum_{j=1}^{20} x_i}{20}=1424.4$$
...
假设$\hat{\theta}$ 是 $\theta$ 的估计量，服从均值为 $\theta$ ，方差为 $\sigma^2/n$ 的渐进正态分布，那么我们可以认为 $g(\hat{\theta})$ 也服从渐进正态分布，其均值和方差分别为：
